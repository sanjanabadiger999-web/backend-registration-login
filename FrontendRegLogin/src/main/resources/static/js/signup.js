const USER_SERVICE_URL = 'http://localhost:8081/api/reg';
...
document.getElementById('signupForm').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    if (response.status === 201) {
      ...
      if (response.status === 409) showAlert('error', message);
      else if (response.status === 400) showAlert('error', message);
      else showAlert('error', message);
    }
  } catch (err) {
    showAlert('error', 'Cannot reach UserService on http://localhost:8081. Is it running?');
  } finally {
